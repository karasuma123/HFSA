...
class Z_id_ex(nn.Module):
    ...
    def forward(self,input,re):
        ...
        # print(x.shape)
        b, _, _, _ = x.shape
        Z, A, lambda_, info, Z_id, eps_out1, eps_out2 = self.iib(x, b)
        if re:
            return Z, A, lambda_, info, Z_id, eps_out1, eps_out2,x
        else:
            return Z_id
...
class Generator_Adain_Upsample(nn.Module):
    def __init__(self, input_nc, output_nc, latent_size, n_blocks=6, deep=False,
        ...
        self.iib = iib()
        self.conv_M = nn.Conv2d(512,512,kernel_size=3,padding=1)
        self.sigmod = nn.Sigmoid()

    def forward(self, input, dlatents):
        Z, A, lambda_, info, Z_id, eps_out1, eps_out2,x = self.Z_id_ex(input,True)
        features = []
        for i in range(len(self.BottleNeck)):
            Z = self.BottleNeck[i](Z, dlatents)
            # bot.append(Z)
        # for i in range(len(self.BottleNeck)):
        # Z = self.BottleNeck[8](Z, dlatents)
        # Z = self.BottleNeck[8](Z, dlatents)
        # Z = self.BottleNeck[8](Z, dlatents)
            # bot.append(Z)
        M = self.sigmod(self.conv_M(x))
        x = M*A + (1.-M)*Z - (1.- lambda_)*eps_out1 - lambda_ * eps_out2
        # print(x.shape)
        if self.deep:
            x = self.up4(x)
            features.append(x)
